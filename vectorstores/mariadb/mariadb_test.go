...
import (
	...
	"github.com/jaysteelmind/langchaingo/vectorstores/mariadb"
...
func TestSimilaritySearchWithDifferentDimensions(t *testing.T) {
	...
	// use openai embedding (now default model is text-embedding-ada-002, with dimensions:1536) to add some data with different dimensions
	llm, err := openai.New(
		openai.WithEmbeddingModel("text-embedding-ada-002"),
	)
	require.NoError(t, err)
	...
	store, err = mariadb.New(
		...
		mariadb.WithVectorDimensions(1536),
		mariadb.WithPreDeleteDatabase(true),
		mariadb.WithEmbeddingTableName("langchain_mariadb_embedding_1536"),
		mariadb.WithCollectionTableName("langchain_mariadb_collection_1536"),
	)
	require.NoError(t, err)

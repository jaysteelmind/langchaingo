...
func TestToolCallProcessing(t *testing.T) {
	...
	require.Equal(t, "call_123", bedrockMsgs[1].ToolCall.ID)
	require.Equal(t, "get_weather", bedrockMsgs[1].ToolCall.Name)
	...
	require.Equal(t, "call_123", bedrockMsgs[2].ToolResult.ToolCallID)
	require.Equal(t, "get_weather", bedrockMsgs[2].ToolResult.ToolName)
	require.Equal(t, "It's sunny and 72°F in New York", bedrockMsgs[2].ToolResult.Content)
}
